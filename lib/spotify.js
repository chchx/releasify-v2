...
export const getArtists = async (refresh_token) => {
  ...
  const res = await explore(access_token)
  // console.log(res)
...
export const getFollowedArtists = async (refresh_token) => {
  return getArtists(refresh_token)
    .then((result) => {
      // console.log('final data is...', result);
      // setArtistIds(result.map((artistData) => artistData.id));
      // console.log('RESULT IS...', result, '...result')
      let mapped = result.map((artistData) => artistData.id);
    ...
    //   // let response;
    //   try {
    //     let response = await fetch(`https://api.spotify.com/v1/artists/${item}/albums`, + new URLSearchParams({
